...
#[cfg(target_arch = "aarch64")]
use kvm_bindings::{
    kvm_one_reg, kvm_regs, user_fpsimd_state, user_pt_regs, RegList, KVM_NR_SPSR, KVM_REG_ARM64,
...
impl cpu::Vcpu for KvmVcpu {
    ...
    ///
    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))]
    fn save_core_registers(&self, state: &mut kvm_regs) -> cpu::Result<()> {
    ...
    ///
    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))]
    fn restore_core_registers(&self, state: &kvm_bindings::kvm_regs) -> cpu::Result<()> {
    ...
    ///
    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))]
    fn save_system_registers(&self, state: &mut Vec<kvm_one_reg>) -> cpu::Result<()> {
    ...
    ///
    #[cfg(any(target_arch = "arm", target_arch = "aarch64"))]
    fn restore_system_registers(&self, state: &[kvm_one_reg]) -> cpu::Result<()> {
        for reg in state {
            self.fd
    ...
    fn state(&self) -> cpu::Result<CpuState> {
        ...
        self.save_core_registers(&mut state.core_regs)?;

        self.save_system_registers(&mut state.sys_regs)?;
    ...
    #[cfg(target_arch = "aarch64")]
    fn set_state(&self, state: &CpuState) -> cpu::Result<()> {
        self.restore_core_registers(&state.core_regs)?;

        self.restore_system_registers(&state.sys_regs)?;
