...
mod tests {
    ...
    fn test_save_restore_core_regs() {
        ...
        let res = vcpu.save_core_registers(&mut state);
        assert!(res.is_err());
        assert_eq!(
        ...
        let res = vcpu.restore_core_registers(&mut state);
        assert!(res.is_err());
        assert_eq!(
            format!("{}", res.unwrap_err()),
        ...
        vcpu.vcpu_init(&kvi).unwrap();
        assert!(vcpu.save_core_registers(&mut state).is_ok());
        assert_eq!(state.regs.pstate, 0x3C5);

        assert!(vcpu.restore_core_registers(&state).is_ok());
    ...
    fn test_save_restore_system_regs() {
        ...
        let res = vcpu.save_system_registers(&mut state);
        assert!(res.is_err());
        assert_eq!(
            format!("{}", res.unwrap_err()),
        ...
        let res = vcpu.restore_system_registers(&mut state);
        assert!(res.is_err());
        assert_eq!(
            format!("{}", res.unwrap_err()),
        ...
        vcpu.vcpu_init(&kvi).unwrap();
        assert!(vcpu.save_system_registers(&mut state).is_ok());
        ...
        assert!(vcpu.restore_system_registers(&state).is_ok());
